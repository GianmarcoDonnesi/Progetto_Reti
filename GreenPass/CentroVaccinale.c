...
void invioRichiestaClient(int sfd, int sfd1) {
    ...
    struct UTENTE richiesta;
    char CodiceTS[21];
    char ris;

    read(sfd,&CodiceTS,sizeof(CodiceTS));
    write(sfd1, "CV",3);
    strcpy(richiesta.numerotessera,CodiceTS);
    fputs("Inserire Codice GreenPass: ", stdout);
    fgets(richiesta.codice, sizeof(richiesta.codice), stdin);
    fflush(stdin);
    fputs("Inserire data validità GreenPass: ", stdout);
    fgets(richiesta.data_scadenza, sizeof(richiesta.data_scadenza), stdin);
    fflush(stdin);
    fputs("Inserire validità: ", stdout);
    fgets(richiesta.validita, sizeof(richiesta.validita), stdin);
    fflush(stdin);
    write(sfd1,&richiesta, sizeof(struct UTENTE));
